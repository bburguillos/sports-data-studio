...
ASSIGNMENT_LEVEL_CODES = {"Rookie": "R", "Pro": "P", "All-Star": "A"}
...
def make_assignment_code(debate_id, level):
    ...
    checksum = zlib.crc32(f"{debate_id}|{level_code}".encode("utf-8")) % 256
    return f"SDS-{claim_number:03d}-{level_code}-{checksum:02X}"


def parse_assignment_code(code):
    original = "".join(str(code or "").split())
    compact = original.upper()
    debate_ids = [debate["id"] for debate in DEBATES]

    # Current short format: SDS-042-P-7A
    if compact.startswith("SDS-"):
        parts = compact.split("-")
        if len(parts) != 4 or not parts[1].isdigit() or parts[2] not in ASSIGNMENT_CODE_LEVELS:
            return None, "That assignment code is incomplete. Use the full code shown by your teacher."
        claim_number = int(parts[1])
        if claim_number < 1 or claim_number > len(debate_ids):
            return None, "That code does not match an available Sports Data Studio claim."
        debate_id = debate_ids[claim_number - 1]
        level = ASSIGNMENT_CODE_LEVELS[parts[2]]
        expected = make_assignment_code(debate_id, level)
        if compact != expected:
            return None, "One or more characters in that code are incorrect. Check the code and try again."
        return {"claim": debate_id, "level": level, "code": expected}, None

    # Continue accepting the longer SDS1 codes generated by version 11.
    if not compact.startswith("SDS1-"):
        return None, "That code should begin with SDS-."
    token = original[5:]
    try:
        token += "=" * (-len(token) % 4)
...
def render_assignment_controls(teacher_mode, mode):
    ...
    if teacher_mode:
        with st.expander("📋 Create a Student Assignment", expanded=True):
    ...
    if active and not teacher_mode:
...
render_assignment_controls(False, mode)
